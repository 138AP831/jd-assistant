 ...
 * - DOCX → unzip in-browser and pull text from word/document.xml
 * - PDF  → send to backend /api/extract-resume (PyPDF2 on the server)
 */
...
async function extractDocx(file: File): Promise<string> {
  ...
  try {
    ...
    const res = await extractPdfViaBackend(file);
    return res.text;
  }
}
...
async function extractPdfViaBackend(file: File): Promise<ExtractionResult> {
  ...
  try {
    ...
    if (e instanceof Error && e.name === "AbortError") {
      throw new Error("PDF extraction timed out. Make sure the backend is running.");
    }
    throw e;
  } finally {
